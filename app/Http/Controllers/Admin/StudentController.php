...
class StudentController extends Controller
{
    ...
    {
        try {
            return $dataTable->render('backend.admin.student.index');
        } catch (\Exception $e) {
    ...
    public function loadCourses(Request $request, CourseRepository $repository): JsonResponse
    {
        try {
            $courses = $repository->activeCourses([
                'user_id'   => $request->id,
    ...
    public function loadCertificates(Request $request, CertificateRepository $certificateRepository, CourseRepository $courseRepository): JsonResponse
    {
        try {
            $courses = $courseRepository->activeCourses([
                'my_course'   => 1,
